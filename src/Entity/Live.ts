...
export default class LivingEntity extends ObjectEntity {
    ...
    protected static applyDamage(entity1: LivingEntity, entity2: LivingEntity) {
        ...
        if (dF1 !== 0) {
            ...
            // Call onDamage for entity2 with recursive owner search
            let trueSource: LivingEntity = entity1;
            while (trueSource.relationsData.values.owner instanceof LivingEntity && trueSource.relationsData.values.owner.hash !== 0) {
                trueSource = trueSource.relationsData.values.owner;
            }
            entity2.onDamage(dF1, trueSource);
        }
        ...
        if (dF2 !== 0) {
            ...
            // Call onDamage for entity1 with recursive owner search
            let trueSource: LivingEntity = entity2;
            while (trueSource.relationsData.values.owner instanceof LivingEntity && trueSource.relationsData.values.owner.hash !== 0) {
                trueSource = trueSource.relationsData.values.owner;
            }
            entity1.onDamage(dF2, trueSource);
        }
