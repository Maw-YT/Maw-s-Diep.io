...
import AutoSTurret from "./AutoSTurret";
...
import { MountedTurretDefinition } from "../Boss/Defender";
...
export class Addon {
    ...
    protected createDefAutoTurrets() {
        ...
        for (let i = 0; i < 3; ++i) {
            ...
            base.positionData.values.y = this.owner.physicsData.values.size * Math.sin(angle) * offset;
            base.positionData.values.x = this.owner.physicsData.values.size * Math.cos(angle) * offset;

            base.physicsData.values.flags |= PositionFlags.absoluteRotation;

            const tickBase = base.tick;
            base.tick = (tick: number) => {
                base.positionData.y = this.owner.physicsData.values.size * Math.sin(angle) * offset;
                base.positionData.x = this.owner.physicsData.values.size * Math.cos(angle) * offset;
    ...
    protected createAutoSTurrets(count: number) {
        const rotPerTick = AI.PASSIVE_ROTATION;
        const MAX_ANGLE_RANGE = PI2 / 4; // keep within 90º each side
...
class AutoSTurretAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        new AutoSTurret(owner);
    }
}
...
/** Custom Defender Auto Turrets */
class DefAutoAddon extends Addon {
    public constructor(owner: BarrelBase) {
        super(owner);

        this.createDefAutoTurrets();
    }
}
