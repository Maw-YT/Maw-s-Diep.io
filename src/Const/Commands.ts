...
import { getTankById, getTankByName } from "./TankDefinitions"
...
export const commandDefinitions = {
    ...
    game_tanks: {
        id: CommandID.gameTanks,
        description: "Lists all tanks and ids, even the dev only ones",
        permissionLevel: AccessLevel.FullAccess,
        isCheat: false
    }
...
export const commandCallbacks = {
    game_set_tank: (client: Client, tankNameArg: string) => {
        ...
        let tankDef
        const tankId = parseInt(tankNameArg, 10)
        if (!isNaN(tankId)) {
            tankDef = getTankById(tankId);
        } else {
            tankDef = getTankByName(tankNameArg);
        }
    ...
    game_tanks: (client: Client) => {
        // Get regular tank definitions
        const tankDefinitions = require("./TankDefinitions").default;
        // Get dev tank definitions
        const devTankDefinitions = require("./DevTankDefinitions").default;
        
        if (!tankDefinitions || !Array.isArray(tankDefinitions)) {
            return "Error: Unable to retrieve tank definitions";
        }
        
        let tankList = "Available Tanks:\n";
        
        // Add regular tanks
        for (let i = 0; i < tankDefinitions.length; i++) {
            const tank = tankDefinitions[i];
            if (tank && tank.name) {
                tankList += `${i}: ${tank.name}${tank.flags?.devOnly ? " (Dev Only)" : ""}\n`;
            }
        }
        
        // Add dev tanks if available
        if (devTankDefinitions && Array.isArray(devTankDefinitions)) {
            tankList += "\nDev Tanks:\n";
            for (let i = 0; i < devTankDefinitions.length; i++) {
                const tank = devTankDefinitions[i];
                if (tank && tank.name && tank.id !== undefined) {
                    // Dev tank IDs are negative, so we show them as negative numbers
                    tankList += `${tank.id}: ${tank.name} (Dev Only)\n`;
                }
            }
        }
        
        saveToLog("", tankList, 0);
        return "look in console for tank list..";
    }
